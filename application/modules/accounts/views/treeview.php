...
function all_subpheadtable($sub_menu){
	foreach($sub_menu as $menu){
		...
		$update='<input name="url" type="hidden" id="url_'.$menu->HeadCode.'" value="'.base_url("accounts/accounts/updatecoa").'" /><a onclick="editinfo('.$menu->HeadCode.')" class="btn btn-info btn-sm" data-toggle="tooltip" data-placement="left" title="Cập nhật"><i class="fa fa-pencil" aria-hidden="true"></i></a>&nbsp;';
		endif; 
		if($ci->permission->method('accounts','delete')->access()): 
		$exitid = $ci->db->select("*")->from('acc_coa')->where('PHeadName',$menu->HeadName)->get()->row();
		if(!empty($exitid)){
			$remove='<a  onclick="return confirm(\''.$alerttxt.'\')" class="btn btn-danger btn-sm" data-toggle="tooltip" data-placement="right" title="Xoá "><i class="fa fa-trash-o" aria-hidden="true"></i></a>'; 
		}
		 else{
			 $remove='<a href="'.base_url("accounts/accounts/deletehead/$menu->HeadCode").'" onclick="return confirm(\''.display("are_you_sure").'\')" class="btn btn-danger btn-sm" data-toggle="tooltip" data-placement="right" title="Xoá "><i class="fa fa-trash-o" aria-hidden="true"></i></a>';
			 }
		endif;
